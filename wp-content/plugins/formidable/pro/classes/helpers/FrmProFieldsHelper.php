...
class FrmProFieldsHelper{
    function FrmProFieldsHelper(){
        add_filter('frm_get_default_value', array(&$this, 'get_default_value'), 10, 2);
            ...
            switch($val){
                ...
                default:
                    ...
                    if (preg_match("/\[get-(.?)\b(.*?)?\]/s", $val)){
                        ...
                        if(is_array($new_value) and !$return_array)
                            $new_value = implode(', ', $new_value);
                    }else if (preg_match('/^\[auto_id/s', $val)){
                        global $frm_entry_meta, $wpdb;
                        
                        $all_entries = $frm_entry_meta->get_entry_metas_for_field($field->id);
                        if($field and isset($field->field_options['post_field']) and $field->field_options['post_field'] == 'post_custom'){
                            $post_values = $wpdb->get_col($wpdb->prepare("SELECT meta_value FROM $wpdb->postmeta WHERE meta_key= %s", $field->field_options['custom_field']));
                            if($post_values)
                                $all_entries = ($all_entries) ? array_merge($all_entries, $post_values) : $post_values;
                        }
                        
                        sort($all_entries);
                        $last_entry = end($all_entries);
                        
                        if($last_entry and is_array($last_entry))
                    ...
                    }else if (preg_match('/^\[user_meta/s', $val)){
                        if(isset($atts['key'])){
                            global $current_user;
                            $new_value = (isset($current_user->{$atts['key']})) ? $current_user->{$atts['key']} : '';
                        }
                    }else if (preg_match('/^\[post_meta/s', $val)){
                        if(isset($atts['key'])){
                            global $post;
                            if($post){
    ...
    function setup_new_vars($values, $field){
        ...
        if(is_array($values['value'])){
            foreach($values['value'] as $val_key => $val)
                $values['value'][$val_key] = FrmProFieldsHelper::get_default_value($val, $field);
        }else if(!empty($values['value'])){
            $values['value'] = FrmProFieldsHelper::get_default_value($values['value'], $field);
        }
...
<?php       }
    ...
    function posted_field_ids($where){
        if (isset($_POST['form_id']) and isset($_POST['frm_page_order_'. $_POST['form_id']]))
            $where .= ' and fi.field_order < '.$_POST['frm_page_order_'. $_POST['form_id']];
        return $where;
    }
...
DEFAULT_HTML;
    ...
    function replace_html_shortcodes($html, $field){
        if ($field['type'] == 'divider'){
            ...
            if(apply_filters('frm_use_wpautop', true))
                $html = wpautop($html);
            $html = $this->get_default_value($html, (object)$field, false);
    ...
    function replace_shortcodes($content, $entry, $shortcodes, $display=false, $show='one', $odd=''){
     ...
     function conditional_replace_with_value($replace_with, $atts, $field, $tag){
         ...
         }else if(isset($atts['equals']) and $atts['equals'] == '' and $replace_with == ''){
