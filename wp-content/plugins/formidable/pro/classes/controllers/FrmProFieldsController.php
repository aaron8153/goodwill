...
class FrmProFieldsController{
    ...
    function use_field_key_value($opt, $opt_key, $field){
        ...
        if(isset($field['use_key']) and $field['use_key'])
            $opt = $opt_key;
        return $opt;
    }
    ...
    function input_html($field){
        ...
        if($frm_settings->use_html){
            if($field['type'] == 'number'){
                ...
                if(!is_numeric($field['step']))
                    $field['step'] = 1;
                echo ' min="'.$field['minnum'].'" max="'.$field['maxnum'].'" step="'.$field['step'].'"';
            }else if(in_array($field['type'], array('url', 'email'))){
                if(!$frm_novalidate and $field['default_value'] == $field['value'])
                    $frm_novalidate = true;
            }
        }
        ...
        if(isset($field['dependent_fields']) and $field['dependent_fields']){
            ...
            echo ' '. $trigger .'="frmCheckDependent(this.value,\''.$field['id'].'\')"';
        }
        ...
        if(is_numeric($selected_field_id)){
            ...
            foreach ($metas as $meta)
                $field['options'][$meta->item_id] = FrmProEntryMetaHelper::display_value($meta->meta_value, $data_field, 
                    array('type' => $data_field->type, 'show_icon' => false, 'show_filename' => false)
                );
